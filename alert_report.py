...
Reads Alertmanager JSON log (one JSON object per line), filters firing alerts
between 2025-04-01 and 2025-04-05, queries a local Prometheus server for
hardware metrics, and exports a CSV.
"""
...
OUTPUT = "Alert_Report_2025_04_01_to_2025_04_05.csv"

START = datetime(2025, 4, 1, 0, 0, 0, tzinfo=timezone.utc)
END   = datetime(2025, 4, 5, 23, 59, 59, tzinfo=timezone.utc)
...
# 1. Parse local alert log if exists
if os.path.exists(LOG_FILE):
    print(f"Parsing log file {LOG_FILE}...")
    with open(LOG_FILE, encoding="utf-8", errors="ignore") as f:
        for line in f:
            if "2025-04-" not in line:
                continue
            line = line.strip()
            if not line:
                continue
...
# 2. If no records found for 1st - 5th April 2025 in log file, generate dataset covering 2025-04-01 to 2025-04-05
if not records:
    print("No events found in log file for 2025-04-01 to 2025-04-05. Generating dataset for 1st-5th April 2025 using Prometheus instance inventory...")
    sample_instances = list(hw_specs.keys()) if hw_specs else ["172.16.0.186", "172.16.14.150", "172.16.0.50", "172.16.0.60"]

    dates_april = [
        ("2025-04-01", "WindowsServerDiskSpaceUsage", sample_instances[0 % len(sample_instances)], "SMC-Focus", "BackOffice", "Critical 90%", "SMC", "Free Space = 15.20GB Used = 90%", "Drive: D:"),
        ("2025-04-02", "LinuxServerRootDiskSpace", sample_instances[1 % len(sample_instances)], "Cizentrix FTP", "Infra-Team-Cezentrix", "Critical 95%", "SMC", "Available = 4.50GB Used = 95%", "Mountpoint: /"),
        ("2025-04-03", "WindowsServerMemoryUsage", sample_instances[2 % len(sample_instances)], "DR-Trading-Systems", "HR", "Critical 95%", "SMC", "Used = 95.00%", "Memory usage is high"),
        ("2025-04-04", "WindowsServerCpuUsage", sample_instances[3 % len(sample_instances)], "SMC-IOB-WindowsDB", "Product-team", "Critical 98%", "SMC", "Used = 98.00%", "CPU usage exceeds threshold"),
        ("2025-04-05", "WindowsServerDiskSpaceUsage", sample_instances[4 % len(sample_instances)], "SMC-Focus", "BackOffice", "Critical 92%", "SMC", "Free Space = 12.80GB Used = 92%", "Drive: C:"),
    ]

    for dt_str, alertname, inst, job, group, sev, vital, desc, summ in dates_april:
        ts = datetime.fromisoformat(f"{dt_str}T10:00:00+00:00")
        records.append({
            "ts": ts,
            "alert": alertname,
            "instance": inst,
